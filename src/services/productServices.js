...
exports.createNewProduct = async (productData) => {
  ...
  if (productData.variants && Array.isArray(productData.variants)) {
    ...
    //   variant price or stock cannot be negative
      if (variant.variantPrice && variant.variantPrice < 0) {
        throw new Error(`Validation Error: Variant price cannot be negative for color: ${variant.color || 'unknown'}.`);
      }
      if (variant.variantStock && variant.variantStock < 0) {
        throw new Error(`Validation Error: Variant stock cannot be negative for color: ${variant.color || 'unknown'}.`);
      }
    }
    };
  ...
  if (existingProduct) {
    if (existingProduct.sku === productData.sku) {
      throw new Error(`Duplicate Error: Product SKU '${productData.sku}' already exists.`);
    }
    if (existingProduct.slug === productData.slug) {
      throw new Error(`Duplicate Error: Product Slug '${productData.slug}' already exists.`);
    }
    };
